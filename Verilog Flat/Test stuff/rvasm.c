...
/* RVASM - RISC-V assembler  Alpha- version  J. Garside  UofM 11/11/21        */
...
// Byte option on Verilog output added 8/4/22
// Fixes & short-form pseudo ops. 11/4/22
...
//sym_table *rv32i_mnemonic_table, *csr_table, *directive_table;
sym_table *rv32i_mnemonic_table, *directive_table;
sym_table *symbol_table;
sym_table_item *rv32i_mnemonic_list, *thumb_mnemonic_list;      /* Real lists */
int i, *j;
...
if (argc == 1)
  {
  printf("RISC V assembler v0.02 (11/11/21)\n");
...
// Returns: 00reg_0off or -1 for error
  ...
  else {offset = 0x000; in_range = TRUE;}
  ...
  if (error_code == eval_okay) return (reg << 16) | (offset & 0xFFF);
  else                         return -1;
  }
...
//        else error_code = SYM_BAD_REG | position;
        ...
        case 0x00040000:                                            /* S-type */
          ...
          if (error_code == eval_okay) value = addr_mode();
          if (value >= 0)
            {
            op_code = op_code | ((value & 0xFE0) << 25)             /* Offset */
                              | ((value & 0x01F) <<  7)
          ...
          switch(token & 0x00F00000)
            ...
            case  0x00000000:
              switch(token & 0xF0000000)
                {
                case  0x00000000: op_code = 0x00000013; break;
                case  0x10000000: op_code = 0x00008067; break;
            ...
            case  0x00200000:
              ...
              switch(token & 0xF00000000)
                {
                case  0x00000000: op_code = op_code | 0x0000006F; break;
  case  0x10000000: op_code = op_code | 0x00000000; break; // CALL placeholder
  case  0x20000000: op_code = op_code | 0x00000000; break; // TAIL placeholder
                default: op_code = 0x00000013; break;
                }
              break;
            ...
            switch (token & 0x00007000)
              ...
              case 0x00000000:                                       /* LI    */
                ...
                if (((value + 0x800) & 0xFFFFF000) == 0)            /* Range? */
                  {
                  flag = TRUE;                 /* Optimise to one instruction */
